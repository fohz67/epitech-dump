/*
** EPITECH PROJECT, 2023
** corewar
** File description:
** construct_buffer_from_file.c
*/

#include <sys/stat.h>
#include <fcntl.h>
#include <unistd.h>
#include <stdlib.h>
#include "asm.h"
#include "lib.h"

static char *free_buf(char *buffer)
{
    if (buffer != NULL)
        free(buffer);
    return NULL;
}

static int file_size(const char *path)
{
    int fd = 0;
    off_t size = 0;

    if ((fd = open(path, O_RDONLY)) == -1)
        return 84;
    if ((size = lseek(fd, 0, SEEK_END)) == -1 || size == 0) {
        if (close(fd) == -1)
            return 84;
        return 84;
    }
    if (close(fd) == -1)
        return 84;
    return (int)size;
}

static int file_check(const char *path, char *buffer, int size)
{
    int fd = 0;

    if ((fd = open(path, O_RDONLY)) == -1)
        return 84;
    if (read(fd, buffer, size) == -1) {
        if (close(fd) == -1)
            return 84;
        return 84;
    }
    if (close(fd) == -1)
        return 84;
    return 0;
}

char *file_content(const char *path)
{
    int size = 0;
    char *buffer = NULL;

    if ((size = file_size(path)) == 84)
        return NULL;
    if ((buffer = malloc(sizeof(char) * (size + 1))) == NULL)
        return NULL;
    if (file_check(path, buffer, size) == 84)
        return free_buf(buffer);
    buffer[size] = '\0';
    return buffer;
}
